...
def texto_a_voz(texto, archivo_salida):
    print(f"🗣️ Generando voz con Google: {texto}")
    # lang='es' es español. Puedes probar 'es-us' para latino o 'es-es' para España.
...
def crear_video():
    print("🎬 Iniciando producción ESTABLE...")
    
    # 1. Preparar el dato
    texto_dato = random.choice(tech_facts)
    
    # 2. Generar Voz (Google TTS)
    texto_a_voz(texto_dato, "voz.mp3")
    ...
    # 4. Audio Final (Voz + Música)
    audios = [clip_voz]
    
    if os.path.exists("musica.mp3"):
        print("🎵 Mezclando música...")
